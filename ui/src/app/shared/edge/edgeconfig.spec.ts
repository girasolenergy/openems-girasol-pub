...
export namespace DummyConfig {
  ...
  export function from(...components: Component[]): EdgeConfig {
    return new EdgeConfig(DUMMY_EDGE, <EdgeConfig>(<unknown>{
      components: <unknown>(
        components?.reduce((acc, c) => ({ ...acc, [c.id]: c }), {})
      ),
  ...
  export function convertDummyEdgeConfigToRealEdgeConfig(
    ...
    components.forEach((obj) => {
      ...
      if (factories[component['factoryId']]) {
        factories[component['factoryId']].componentIds = [
          ...factories[component['factoryId']].componentIds,
          ...component['factory'].componentIds,
        ];
      } else {
...
export const LINE_INFO_PHASES_DE: OeFormlyViewTester.Field = {
  type: 'info-line',
  name: '技術的な理由により、個々のフェーズの合計は、合計の合計とは異なることがあります。',
};
