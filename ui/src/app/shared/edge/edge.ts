...
export class Edge {
  ...
  // holds config
  private config: BehaviorSubject<EdgeConfig> = new BehaviorSubject<EdgeConfig>(
    null
  );
  ...
  private refreshConfig(websocket: Websocket): void {
    ...
    this.sendRequest(websocket, request)
      .then((response) => {
        console.log('获得返回更新组件配置的全部数据', request);
  ...
  public unsubscribeFromChannels(
    ...
    this.subscribedChannels = Object.entries(this.subscribedChannels).reduce(
      (arr, [id, subscribedChannels]) => {
        if (
          ArrayUtils.equalsCheck(
  ...
  public createComponentConfig(
    ...
    let request = new CreateComponentConfigRequest({
  ...
  public updateComponentConfig(
    ...
    let request = new UpdateComponentConfigRequest({
      componentId: componentId,
      properties: properties,
    });
    console.log('发送了更新组件配置的请求', request);
    return this.sendRequest(ws, request);
  }
  ...
  public deleteComponentConfig(
    ...
    let request = new DeleteComponentConfigRequest({
      componentId: componentId,
    });
    return this.sendRequest(ws, request);
  }
  ...
  public sendRequest(
    ...
    console.log(9990, '基本方法请求', request);
    let wrap = new EdgeRpcRequest({ edgeId: this.id, payload: request });
    console.log(9991, '包成EdgeRpc请求', wrap);
    return new Promise((resolve, reject) => {
      ws.sendRequest(wrap)
        .then((response) => {
          console.log(9992, '结果', response);
