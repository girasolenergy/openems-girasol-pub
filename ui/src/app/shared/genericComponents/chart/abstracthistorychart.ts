...
import {
  ...
  DEFAULT_TIME_CHART_OPTIONS_WITHOUT_PREDEFINED_Y_AXIS,
  isLabelVisible,
  setLabelVisible,
  TooltipItem,
  Unit,
  Resolution,
} from '../../../edge/history/shared';
...
export abstract class AbstractHistoryChart implements OnInit {
  ...
  protected static phaseColors: string[] = [
    'rgb(255,127,80)',
    'rgb(0,0,255)',
    'rgb(128,128,0)',
  ];
  ...
  public labels: Date[] = [];
  public datasets: ChartDataSets[] = HistoryUtils.createEmptyDataset(
    this.translate
  );
  ...
  private refreshChartData = interval(60000);
  ...
  protected updateChart() {
    ...
    } else if (this.hasSubscribed == true) {
  ...
  public static fillChart(
    ...
    let colors: any[] = [];
    let displayValues: HistoryUtils.DisplayValues[] = chartObject.output(
      channelData.data
    );
    let legendOptions: {
      label: string;
    ...
    displayValues.forEach((element, index) => {
      ...
      let yAxis = chartObject.yAxes.find(
        (yaxis) =>
          yaxis?.yAxisId == (element?.yAxisId ?? chartObject.yAxes[0]?.yAxisId)
      );

      // Filter existing values
      if (element) {
        let label = AbstractHistoryChart.getTooltipsLabelName(
          element.name,
          yAxis?.unit,
          nameSuffix
        );
        ...
        let configuration = AbstractHistoryChart.fillData(
          element,
          label,
          chartObject,
          chartType,
  ...
  public static fillData(
    element: HistoryUtils.DisplayValues,
    label: string,
    chartObject: HistoryUtils.ChartData,
    chartType: 'line' | 'bar',
    data: number[] | null
    ...
    if (unit == Unit.DAYS || unit == Unit.MONTHS) {
      ...
      Promise.all([
        ...
        .then(([energyPeriodResponse, energyResponse]) => {
          ...
          if (unit === Unit.MONTHS) {
            energyPeriodResponse.result.timestamps[0] =
              startOfMonth(
                DateUtils.stringToDate(
                  energyPeriodResponse.result.timestamps[0]
                )
              )?.toString() ?? energyPeriodResponse.result.timestamps[0];
          }
          let displayValues = AbstractHistoryChart.fillChart(
            this.chartType,
            this.chartObject,
            energyPeriodResponse,
            energyResponse
  ...
  protected queryHistoricTimeseriesData(
    ...
    let resolution =
      res ?? calculateResolution(this.service, fromDate, toDate).resolution;
    this.errorResponse = null;
    let result: Promise<QueryHistoricTimeseriesDataResponse> =
      new Promise<QueryHistoricTimeseriesDataResponse>((resolve, reject) => {
        this.service.getCurrentEdge().then((edge) => {
          this.service.getConfig().then(async () => {
            let channelAddresses = (await this.getChannelAddresses())
              .powerChannels;
            ...
            edge
              ...
              .then((response) => {
                ...
                if (Object.keys(result).length != 0) {
                  ...
                  resolve(
                    new QueryHistoricTimeseriesDataResponse(response.id, {
                      timestamps: [null],
                      data: { null: null },
                    })
                  );
  ...
  protected queryHistoricTimeseriesEnergyPerPeriod(
    ...
    let result: Promise<QueryHistoricTimeseriesEnergyPerPeriodResponse> =
        ...
        (resolve, reject) => {
          this.service.getCurrentEdge().then((edge) => {
            this.service.getConfig().then(async () => {
              ...
              ).energyChannels.filter((element) => element != null);
                ...
                edge
                  ...
                  .then((response) => {
                    ...
                    if (Object.keys(result).length != 0) {
                      ...
                      this.errorResponse = new JsonrpcResponseError(
                        request.id,
                        { code: 1, message: 'Empty Result' }
                      );
                      resolve(
  ...
  protected queryHistoricTimeseriesEnergy(
    ...
    let result: Promise<QueryHistoricTimeseriesEnergyResponse> =
      new Promise<QueryHistoricTimeseriesEnergyResponse>((resolve, reject) => {
        this.service.getCurrentEdge().then((edge) => {
          this.service.getConfig().then(async () => {
            let channelAddresses =
              (await this.getChannelAddresses()).energyChannels?.filter(
  ...
  public static getOptions(
    ...
    let options = Utils.deepCopy(
      <ChartOptions>(
        Utils.deepCopy(DEFAULT_TIME_CHART_OPTIONS_WITHOUT_PREDEFINED_Y_AXIS)
      )
    );
    ...
    chartObject.yAxes.forEach((element) => {
      ...
      tooltipsLabel = AbstractHistoryChart.getToolTipsLabel(
        element.unit,
        chartType
      );
    });
    ...
    if (chartType == 'bar') {
      ...
      options.tooltips.callbacks.afterTitle = function (
        ...
        let totalValue = items
          .filter((element) => !element.label.includes(afterTitle))
        ...
        if (afterTitle) {
          return (
            afterTitle +
            ': ' +
            formatNumber(totalValue, 'en', chartObject.tooltip.formatNumber) +
            ' ' +
            tooltipsLabel
          );
        }
    ...
    options.tooltips.callbacks.label = (
      ...
      let displayValue = displayValues.find(
        (element) => element.name === label.split(':')[0]
      );
      let unit = displayValue?.customUnit ?? chartObject.yAxes[0]?.unit;
      ...
      return (
        label.split(':')[0] +
        ': ' +
        formatNumber(value, 'en', chartObject.tooltip.formatNumber) +
        ' ' +
        tooltipsLabel
      );
    };
    ...
    options.legend.labels.generateLabels = function (chart: Chart) {
      ...
      chart.data.datasets.forEach((dataset, index) => {
        let legendItem = legendOptions?.find(
        ...
        displayValues
          .filter((element) => element.name == dataset.label?.split(':')[0])
          .forEach(() => {
            chartLegendLabelItems.push({
              text: dataset.label.split(':')[0], // 只保留标签中的文字
              datasetIndex: index,
              fillStyle: dataset.backgroundColor.toString(),
              hidden:
                isHidden != null ? isHidden : !chart.isDatasetVisible(index),
              lineWidth: 2,
    ...
    Chart.defaults.global.legend.onClick = function (
      ...
      let legendItems = Chart.defaults.global.legend.labels.generateLabels(
        this.chart
      );
  ...
  private getChannelAddresses(): Promise<{
    ...
    return new Promise<{
      ...
      if (this.chartObject?.input) {
        resolve({
          powerChannels: this.chartObject.input.map(
            (element) => element.powerChannel
          ),
          energyChannels: this.chartObject.input.map(
   ...
   * @returns
   */
  private static getToolTipsLabel(
    title: YAxisTitle,
    chartType: 'bar' | 'line'
  ): string {
    switch (title) {
        ...
        switch (unit) {
          case YAxisTitle.ENERGY:
            return (
              baseName +
              ': ' +
              formatNumber(suffix / 1000, 'en', '1.0-1') +
              ' kWh'
            );
          case YAxisTitle.PERCENTAGE:
            return baseName + ': ' + formatNumber(suffix, 'en', '1.0-1') + ' %';
        }
      }
    }
