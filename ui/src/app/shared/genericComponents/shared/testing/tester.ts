...
export class OeFormlyViewTester {
  ...
  private static applyField(
    ...
    switch (field.type) {
      ...
      case 'children-line':
        ...
        if (field.children) {
          result.children = field.children?.map((child) =>
            OeFormlyViewTester.applyField(child, context)
          );
        }
      ...
      case 'value-from-channels-line': {
        const tmp = OeFormlyViewTester.applyValueLineFromChannels(
          field,
          context
        );
        if (tmp == null) {
  ...
  private static applyLineOrItem(
    ...
    context: OeFormlyViewTester.Context
  ): /* result */ { rawValue: number | null; value: string } | null {
    /* filter did not pass */
    // Read value from channels
    let rawValue =
      field.channel && field.channel in context ? context[field.channel] : null;
...
export class OeChartTester {
  public static apply(
    ...
    let channelData = OeChartTester.getChannelDataByCharttype(
      chartType,
      channels
    );
    ...
    testContext.service.historyPeriod.next({
      ...
      getText: () =>
        testContext.service.historyPeriod.value.getText(testContext.translate),
    });

    // Fill Data
    let configuration = AbstractHistoryChart.fillChart(
      chartType,
      chartData,
      channelData,
      channels.energyChannelWithValues
    );
    let data: OeChartTester.Dataset.Data[] =
      OeChartTester.convertChartDatasetsToDatasets(configuration.datasets);
    let labels: OeChartTester.Dataset.LegendLabel =
      OeChartTester.convertChartLabelsToLegendLabels(configuration.labels);
    let options: OeChartTester.Dataset.Option =
      OeChartTester.convertChartDataToOptions(
        chartData,
        chartType,
        testContext,
        channels
   ...
   */
  public static convertChartLabelsToLegendLabels(
    labels: Date[]
  ): OeChartTester.Dataset.LegendLabel {
    return {
      type: 'label',
      timestamps: labels,
    };
   ...
   */
  public static convertChartDatasetsToDatasets(
    datasets: ChartDataSets[]
  ): OeChartTester.Dataset.Data[] {
  ...
  public static convertChartDataToOptions(
    ...
    displayValues.forEach((displayValue) => {
      let yAxis = chartData.yAxes.find(
        (yaxis) =>
          yaxis?.yAxisId ==
          (displayValue?.yAxisId ?? chartData.yAxes[0].yAxisId)
      );
      ...
      legendOptions.push(
        AbstractHistoryChart.getLegendOptions(label, displayValue)
      );
    });

    return {
      type: 'option',
      options: AbstractHistoryChart.getOptions(
        chartData,
        chartType,
        testContext.service,
