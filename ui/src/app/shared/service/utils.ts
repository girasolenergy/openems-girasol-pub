...
export class Utils {
  ...
  public static compareArraysSafely(v1: any[], v2: any[]): boolean {
    ...
    return (
      v1.every((item) => set2.has(item)) && v2.every((item) => set1.has(item))
    );
  }
  ...
  public static CONVERT_TO_WATT = (value: any): string => {
    if (value == null) {
      ...
      return formatNumber(value, 'en', '1.0-0') + ' W';
    } else {
      return '0 W';
    }
  };
  ...
  public static CONVERT_TO_VOLT = (value: any): string => {
    if (value == null) {
      ...
      return formatNumber(value / 1000, 'en', '1.0-0') + ' V';
    } else {
      return '0 V';
    }
  };
  ...
  public static CONVERT_TO_CURRENT = (value: any): string => {
    if (value == null) {
      ...
      return formatNumber(value / 1000, 'en', '1.1-1') + ' A';
    } else {
      return '0 A';
    }
  };
  ...
  /**
   * Converts a value to Watt per Square Meter [W/m²]
   *
   * @param value the value from passed value in html
   * @returns converted value
   */
  public static CONVERT_TO_LUX = (value: any): string => {
    // Divide the value by 10 and format it with 'W/m²' unit

    return formatNumber(value, 'en', '1.0-1') + ' W/m²';
  };

  /**
   * Converts a value to Degrees Celsius [°C]
   *
   * @param value the value from passed value in html
   * @returns converted value
   */
  public static CONVERT_TO_DEGREES = (value: any): string => {
    // Divide the value by 10 and format it with '°C' unit
    const convertedValue = Utils.divideSafely(value, 10);
    return formatNumber(convertedValue, 'en', '1.0-1') + ' °C';
  };

  /**
   * Converts a value in Watt [W] to KiloWatt [kW] to kG.
   *
   * @param value the value from passed value in html
   * @returns converted value
   */
  public static CONVERT_WATT_TO_KG = (value: any): string => {
    if (value == null) {
      return '-';
    }
    let thisValue: number = (value / 1000) * 0.623;

    if (thisValue >= 0) {
      return formatNumber(thisValue, 'en', '1.0-1') + ' kg';
    } else {
      return '0 kW';
    }
  };
  ...
  public static CONVERT_TO_WATTHOURS = (value: any): string => {
    return formatNumber(value, 'en', '1.0-1') + ' Wh';
  };
  ...
  public static CONVERT_TO_KILO_WATTHOURS = (value: any): string => {
    return (
      formatNumber(Utils.divideSafely(value, 1000), 'en', '1.0-1') + ' kWh'
    );
  };
   ...
   * @returns converted value
   */
  public static CONVERT_MINUTE_TO_TIME_OF_DAY = (
    translate: TranslateService
  ) => {
    return (value: number): string => {
      ...
      return date.toLocaleTimeString(translate.getBrowserCultureLang(), {
        hour: '2-digit',
        minute: '2-digit',
      });
    };
  ...
  public static CONVERT_PRICE_TO_CENT_PER_KWH = (
    ...
    return (value: any): string =>
      (!value ? '-' : formatNumber(value / 10, 'en', '1.0-' + decimal)) +
      ' ' +
      label;
  };
      ...
      switch (value) {
        case -1:
          return translate.instant(
            'Edge.Index.Widgets.TimeOfUseTariff.State.notStarted'
          );
        case 0:
          return translate.instant(
            'Edge.Index.Widgets.TimeOfUseTariff.State.delayed'
          );
        case 1:
          return translate.instant(
            'Edge.Index.Widgets.TimeOfUseTariff.State.allowsDischarge'
          );
        case 2:
          return translate.instant(
            'Edge.Index.Widgets.TimeOfUseTariff.State.standby'
          );
        case 3:
          return translate.instant(
            'Edge.Index.Widgets.TimeOfUseTariff.State.CHARGING'
          );
      }
    };
  };
  ...
  public static downloadXlsx(
    ...
    var binary = Buffer.from(
      response.result.payload.replace(/\s/g, ''),
      'base64'
    );
    ...
    // }
    const data: Blob = new Blob([binary], {
  ...
  public static isDataEmpty(arg: JsonrpcResponseSuccess): boolean {
    return Object.values(arg.result['data'])
      ?.map((element) => element as number[])
      ?.every((element) => element?.every((elem) => elem == null) ?? true);
  }
   ...
   * @returns the label.
   */
  public static getTimeOfUseTariffStorageLabel(
    component: EdgeConfig.Component,
    translate: TranslateService
  ): string {
    if (component.factoryId === 'Controller.Ess.Time-Of-Use-Tariff.Discharge') {
      return translate.instant(
        'Edge.Index.Widgets.TimeOfUseTariff.STORAGE_DISCHARGE'
      );
    } else {
      return translate.instant(
        'Edge.Index.Widgets.TimeOfUseTariff.STORAGE_STATUS'
      );
    }
  }
}
