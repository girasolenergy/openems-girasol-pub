...
export class Utils {
  ...
  public static CONVERT_WATT_TO_KG = (value: any): string => {
    ...
    const thisValue: number = (value / 1000) * 0.623;
...
export namespace TimeOfUseTariffUtils {
  ...
  export function getLabel(value: number, label: string, translate: TranslateService, currencyLabel?: Currency.Label): string {
    ...
    switch (label) {
      case socLabel:
        return label + ": " + formatNumber(value, "en", "1.0-0") + " %";

      case dischargeLabel:
      case chargeConsumptionLabel:
      case balancingLabel:
        // Show floating point number for values between 0 and 1
        return label + ": " + formatNumber(value, "en", "1.0-4") + " " + currencyLabel;


      default:
      case gridBuyLabel:
        // Power values
        return label + ": " + formatNumber(value, "en", "1.0-2") + " " + " kW";
    }
  }
