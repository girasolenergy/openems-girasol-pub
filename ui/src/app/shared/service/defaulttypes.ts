...
export module DefaultTypes {
  ...
  export class HistoryPeriod {
    constructor(public from: Date = new Date(), public to: Date = new Date()) {}
    ...
    public getText(translate: TranslateService): string {
      if (isSameDay(this.from, this.to)) {
        if (isSameDay(this.from, new Date())) {
          // Selected TODAY
          return (
            translate.instant('Edge.History.today') +
            ', ' +
            format(new Date(), translate.instant('General.dateFormat'))
          );
        } else if (isSameDay(this.from, subDays(new Date(), 1))) {
          // Selected YESTERDAY
          return (
            translate.instant('Edge.History.yesterday') +
            ', ' +
            format(this.from, translate.instant('General.dateFormat'))
          );
        } else {
          // Selected one single day
          return (
            HistoryPeriod.getTranslatedDayString(translate, this.from) +
            ', ' +
            translate.instant('Edge.History.selectedDay', {
              value: format(this.from, translate.instant('General.dateFormat')),
            })
          );
        }
      } else if (
        isSameMonth(this.from, this.to) &&
        isSameDay(this.from, startOfMonth(this.from)) &&
        isSameDay(this.to, endOfMonth(this.to))
      ) {
        // Selected one month
        return (
          HistoryPeriod.getTranslatedMonthString(translate, this.from) +
          ' ' +
          getYear(this.from)
        );
      }
      // Selected one year
        ...
        isSameYear(this.from, this.to) &&
        isSameDay(this.from, startOfYear(this.from)) &&
        isSameDay(this.to, endOfYear(this.to))
      ) {
        return getYear(this.from).toString();
      } else {
        return translate.instant('General.periodFromTo', {
          value1: format(
            this.from,
            translate.instant('General.dateFormatShort')
          ),
     ...
     * @param date the Date
     */
    private static getTranslatedDayString(
      translate: TranslateService,
      date: Date
    ): string {
      switch (getDay(date)) {
        case 0:
          return translate.instant('General.Week.sunday');
        case 1:
          return translate.instant('General.Week.monday');
        case 2:
          return translate.instant('General.Week.tuesday');
        case 3:
          return translate.instant('General.Week.wednesday');
        case 4:
          return translate.instant('General.Week.thursday');
        case 5:
          return translate.instant('General.Week.friday');
        case 6:
          return translate.instant('General.Week.saturday');
      }
    }
     ...
     * @param date the Date
     */
    private static getTranslatedMonthString(
      translate: TranslateService,
      date: Date
    ): string {
      switch (getMonth(date) + 1) {
        case 1:
          return translate.instant('General.Month.january');
        case 2:
          return translate.instant('General.Month.february');
        case 3:
          return translate.instant('General.Month.march');
        case 4:
          return translate.instant('General.Month.april');
        case 5:
          return translate.instant('General.Month.may');
        case 6:
          return translate.instant('General.Month.june');
        case 7:
          return translate.instant('General.Month.july');
        case 8:
          return translate.instant('General.Month.august');
        case 9:
          return translate.instant('General.Month.september');
        case 10:
          return translate.instant('General.Month.october');
        case 11:
          return translate.instant('General.Month.november');
        case 12:
          return translate.instant('General.Month.december');
      }
    }
  }
}
