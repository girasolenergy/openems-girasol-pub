...
export class Websocket implements WebsocketInterface {
  ...
  public login(request: AuthenticateWithPasswordRequest | AuthenticateWithTokenRequest): Promise<void> {
    return new Promise<void>((resolve) => {
      this.sendRequest(request).then(r => {
        ...
        //set user for sentry
        setUser({
          id: authenticateResponse.user.id,
          email: authenticateResponse.user.id,
          username: authenticateResponse.user.name,
        });
  ...
  */
    ...
    this.socket = webSocket({
      ...
      openObserver: {
        next: (value) => {
          ...
          // let defaultUser = { username: '', password: '' };
          // this.login(new AuthenticateWithPasswordRequest(defaultUser));
  ...
  private checkErrorCode(reason: JsonrpcResponseError): void {

    // TODO create global Errorhandler for any type of error
    const sendErrorToSentry = (code: number, message: string): void => {
    ...
    switch (reason.error.code) {
      case 1003:
        ...
        this.onLoggedOut();
        sendErrorToSentry(reason.error.code, reason.error.message || "Authentication failed");
        break;
      case 1:
        this.service.toast(this.translate.instant("Login.REQUEST_TIMEOUT"), "danger");
        this.status = "waiting for credentials";
        this.service.onLogout();
        sendErrorToSentry(reason.error.code, reason.error.message || "Request timeout");
        break;
      default: {
        const errorMessage = `${reason.error.code}: ${reason.error?.message}`;
        if (errorMessage) {
          console.log(errorMessage);
          this.service.toast(errorMessage, "danger");
        }
        this.onLoggedOut();
        sendErrorToSentry(reason.error.code, reason.error.message || "Default Error");
      }
    }
  }
