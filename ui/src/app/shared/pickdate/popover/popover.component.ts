import { CalAnimation, IAngularMyDpOptions, IMyDate, IMySingleDateModel } from 'angular-mydatepicker';
...
export class PickDatePopoverComponent implements OnInit {
    ...
    private readonly YESTERDAY = addDays(new Date(), -1);
    ...
    public onDateChanged(event: IMySingleDateModel) {
        this.popoverCtrl.dismiss();
        const selectedDate = event.jsDate;
        if (selectedDate) {
          this.setDateRange(new DefaultTypes.HistoryPeriod(selectedDate, selectedDate));
