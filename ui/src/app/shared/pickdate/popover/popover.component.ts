...
export class PickDatePopoverComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.locale =Language.getByKey(localStorage.LANGUAGE).i18nLocaleKey;
        // Filter out custom due to different on click event
        this.periods = this.showOnlyDay ? [DefaultTypes.PeriodString.DAY] :EdgePermission.getAllowedHistoryPeriods(this.edge).filter(period => period !== DefaultTypes.PeriodString.CUSTOM);
      }
