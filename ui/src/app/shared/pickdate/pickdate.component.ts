...
export class PickDateComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit() {
    ...
    this.service.getCurrentEdge().then((edge) => {
      ...
      this.isForwardArrowAllowed = PickDateComponent.isNextPeriodAllowed(
        this.service
      );
    });
  }
    ...
    switch (this.service.periodString) {
      ...
      case DefaultTypes.PeriodString.CUSTOM: {
        let dateDistance = Math.floor(
          Math.abs(
            <any>this.service.historyPeriod.value.from -
              <any>this.service.historyPeriod.value.to
          ) /
            (1000 * 60 * 60 * 24)
        );
        ...
        if (
          isFuture(
            addDays(this.service.historyPeriod.value.from, dateDistance * 2)
          )
        ) {
          this.disableArrow = true;
        } else {
  ...
  public goForward() {
    switch (this.service.periodString) {
        ...
        if (isFuture(addWeeks(this.service.historyPeriod.value.from, 2))) {
          ...
          this.setDateRange(
            new DefaultTypes.HistoryPeriod(
              ...
              addWeeks(
                endOfWeek(this.service.historyPeriod.value.to, {
                  weekStartsOn: 1,
                }),
                1
              )
      ...
      case DefaultTypes.PeriodString.CUSTOM: {
        ...
        if (
          isFuture(
            addDays(this.service.historyPeriod.value.to, dateDistance * 2)
          )
        ) {
          this.disableArrow = true;
        }
        if (
          !isFuture(addDays(this.service.historyPeriod.value.to, dateDistance))
        ) {
          this.setDateRange(
            new DefaultTypes.HistoryPeriod(
  ...
  private forwardToNextMonthWhenReached() {
    ...
    if (this.millisecondsUntilnextPeriod() < 2147483647) {
      this.changePeriodTimeout = setTimeout(() => {
        this.setDateRange(
          new DefaultTypes.HistoryPeriod(new Date(), endOfMonth(new Date()))
        );
        this.service.historyPeriod.value?.getText(this.translate);
      }, this.millisecondsUntilnextPeriod());
    }
  ...
  private forwardToNextYearWhenReached() {
    ...
    if (this.millisecondsUntilnextPeriod() < 2147483647) {
      this.changePeriodTimeout = setTimeout(() => {
        this.setDateRange(
          new DefaultTypes.HistoryPeriod(new Date(), endOfYear(new Date()))
        );
        this.service.historyPeriod.value?.getText(this.translate);
      }, this.millisecondsUntilnextPeriod());
    }
    ...
    switch (service.periodString) {
      case DefaultTypes.PeriodString.DAY:
        return isBefore(
          ...
          endOfDay(subDays(service.historyPeriod.value.from, 1))
        );
      case DefaultTypes.PeriodString.WEEK:
        return isBefore(
          firstSetupProtocol,
          endOfWeek(subWeeks(service.historyPeriod.value.from, 1))
        );
      case DefaultTypes.PeriodString.MONTH:
        return isBefore(
          firstSetupProtocol,
          endOfMonth(subWeeks(service.historyPeriod.value.from, 1))
        );
      case DefaultTypes.PeriodString.YEAR:
        return isBefore(
          firstSetupProtocol,
          endOfYear(subWeeks(service.historyPeriod.value.from, 1))
        );
      case DefaultTypes.PeriodString.CUSTOM:
        var timeRange: number = differenceInDays(
  ...
  public static isNextPeriodAllowed(service: Service): boolean {
    switch (service.periodString) {
      case DefaultTypes.PeriodString.DAY:
        return isAfter(
          new Date(),
          startOfDay(addDays(service.historyPeriod.value.to, 1))
        );
      case DefaultTypes.PeriodString.WEEK:
        return isAfter(
          new Date(),
          startOfDay(
            startOfWeek(addWeeks(service.historyPeriod.value.to, 1), {
              weekStartsOn: 1,
            })
          )
        );
      case DefaultTypes.PeriodString.MONTH:
        return isAfter(
          new Date(),
          startOfMonth(addMonths(service.historyPeriod.value.to, 1))
        );
      case DefaultTypes.PeriodString.YEAR:
        return isAfter(
          new Date(),
          startOfYear(addYears(service.historyPeriod.value.to, 1))
        );
      case DefaultTypes.PeriodString.CUSTOM:
        var timeRange: number = differenceInDays(
          service.historyPeriod.value.to,
        ...
        return isAfter(
          startOfDay(new Date()),
          addDays(service.historyPeriod.value.to, timeRange)
        );
    }
  }
}
