...
export class HeaderComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  public translatedLiveLabel: string;
  public translatedHistoryLabel: string;
  ...
  ngOnInit() {
    this.translateService
      ...
      .subscribe((translation: string) => {
        this.translatedLiveLabel = translation;
      });
    this.translateService
      .get('General.history')
      .subscribe((translation: string) => {
        this.translatedHistoryLabel = translation;
      });
    this.translateService
      .get('General.meter')
      .subscribe((translation: string) => {
        this.translatedMeterLabel = translation;
      });
    // set inital URL
  ...
  updateCurrentPage(url: string) {
    ...
    if (
      (file == 'history' || file == 'live' || file == 'meters') &&
      urlArray.length == 3
    ) {
      if (file == 'history') {
  ...
  public segmentChanged(event) {
    if (event.detail.value == 'IndexLive') {
      this.router.navigateByUrl(
        '/device/' + this.service.currentEdge.value.id + '/live',
        { replaceUrl: true }
      );
      this.cdRef.detectChanges();
    }
    ...
    //在导航到路由时，你可以使用 router.navigate 方法传递查询参数：
    if (event.detail.value == 'IndexMeter') {
      this.router.navigateByUrl(
        '/device/' + this.service.currentEdge.value.id + '/meters',
        { replaceUrl: true }
      );
      this.cdRef.detectChanges();
    }
