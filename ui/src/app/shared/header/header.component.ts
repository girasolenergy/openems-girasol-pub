...
export class HeaderComponent implements OnInit, OnDestroy, AfterViewChecked {
    ...
    public currentPage: 'EdgeSettings' | 'Other' | 'IndexLive' | 'IndexHistory' |  'IndexSignage' | 'IndexMeter' | 'IndexHistorySignage' = 'Other';
    public isSystemLogEnabled: boolean = false;
    private ngUnsubscribe: Subject<void> = new Subject<void>();

    public translatedMeterLabel: string;
    public showOnlyDay = true;
    constructor(
    ...
    updateCurrentPage(url: string) {
        ...
        if ((file == 'history' || file == 'live' || file == 'meters' || file == 'signage'|| file == 'historysignage') && urlArray.length == 3) {
            if (file == 'history') {
                this.currentPage = 'IndexHistory';
                this.showOnlyDay = false;
            } else if (file == 'meters') {
                this.currentPage = 'IndexMeter';
                this.showOnlyDay = true;
            } else if (file == 'signage') {
                this.currentPage = 'IndexSignage';
            } else if (file == 'historysignage'){
                this.currentPage = 'IndexHistorySignage';
                this.showOnlyDay = true;
            } else {
    ...
    public segmentChanged(event) {
        ...
        if (event.detail.value == 'IndexMeter') {
          this.router.navigate(['../meters'], { relativeTo: this.route });
            this.cdRef.detectChanges();
        }
        if (event.detail.value == "IndexSignage") {
          this.router.navigateByUrl("/device/" + this.service.currentEdge.value.id + "/signage", { replaceUrl: true });
          this.cdRef.detectChanges();
        }
        if (event.detail.value == "IndexHistorySignage") {
          this.router.navigate(['../historysignage'], { relativeTo: this.route });
          this.cdRef.detectChanges();
        }
    }
