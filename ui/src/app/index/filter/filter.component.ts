...
export class FilterComponent {
  @Output() protected setSearchParams: EventEmitter<
    Map<string, ChosenFilter['value']>
  > = new EventEmitter<Map<string, ChosenFilter['value']>>();
  ...
  public searchOnChange(event, filter: Filter): void {
    ...
    if (Array.isArray(this.searchParams.get(filter.category))) {
      didFilterChange = Utils.compareArraysSafely(
        value,
        this.searchParams.get(filter.category) as any[]
      );
    } else {
