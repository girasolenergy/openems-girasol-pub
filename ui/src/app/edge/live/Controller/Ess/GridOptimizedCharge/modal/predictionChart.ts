...
import {
  ...
  TooltipItem,
  Unit,
} from 'src/app/edge/history/shared';
  ...
  implements OnInit, OnChanges, OnDestroy
{
  ...
  private static DEFAULT_PERIOD: DefaultTypes.HistoryPeriod =
      ...
      .then((response) => {
        ...
        if ('_sum/EssSoc' in result.data) {
          ...
          if (isChargeStartPresent) {
            ...
            chargeStartIndex = Math.trunc(
              (chargeStartHours * 60 + chargeStartMinutes) / 5
            );
          }
          ...
          datasets.push(
            ...
            },
            {
              label: this.translate.instant(
                'Edge.Index.Widgets.GridOptimizedCharge.expectedSoc'
              ),
  ...
  protected getChannelAddresses(): Promise<ChannelAddress[]> {
    return new Promise((resolve) => {
      let result: ChannelAddress[] = [new ChannelAddress('_sum', 'EssSoc')];
