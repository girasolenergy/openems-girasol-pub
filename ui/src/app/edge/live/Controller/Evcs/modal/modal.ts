...
type SetLockState = 'LOCKED' | 'UNLOCKED';
...
export class ModalComponent implements OnInit {
  ...
  public setLockState: SetLockState = null;
  ...
  getLockState(state: number): string {
    let lockState = state;
    switch (lockState) {
      case 0:
        return this.translate.instant('Edge.Index.Widgets.EVCS.lock');
      case 1:
        return this.translate.instant('Edge.Index.Widgets.EVCS.unlock');
    }
  }
  /**
  ...
  updateChargeMode(
    ...
    if (this.edge != null) {
      this.edge
        .updateComponentConfig(this.websocket, currentController.id, [
          { name: 'chargeMode', value: newChargeMode },
          { name: 'lockState', value: newSetLockState },
        ...
        .then(() => {
          ...
          this.service.toast(
            this.translate.instant('General.changeAccepted'),
            'success'
          );
        })
        .catch((reason) => {
          ...
          this.service.toast(
            this.translate.instant('General.changeFailed') +
              '\n' +
  ...
  updateDefaultMinPower(
    ...
    if (this.edge != null) {
      this.edge
        ...
        .then(() => {
          currentController.properties.defaultChargeMinPower =
            newMinChargePower;
          this.service.toast(
            this.translate.instant('General.changeAccepted'),
            'success'
   ...
   * @param phases
   */
  allowMinimumChargePower(
    phases: number,
    currentController: EdgeConfig.Component
  ) {
    let oldMinChargePower =
      currentController.properties['defaultChargeMinPower'];
  ...
  getNumberOfPhasesOrThree() {
    let numberOfPhases =
      this.edge.currentData['_value'].channel[this.componentId + '/Phases'];
