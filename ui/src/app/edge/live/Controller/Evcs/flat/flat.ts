...
export class FlatComponent extends AbstractFlatWidget {
  ...
  public readonly CONVERT_MANUAL_ON_OFF = Utils.CONVERT_MANUAL_ON_OFF(
    this.translate
  );
  ...
  protected readonly CONVERT_TO_KILO_WATTHOURS =
    Utils.CONVERT_TO_KILO_WATTHOURS;
  protected readonly CONVERT_MANUAL_ON_OFF_AUTOMATIC =
  ...
  protected override getChannelAddresses(): ChannelAddress[] {
    return [
      ...
      new ChannelAddress(this.component.id, 'SetLockState'),
      new ChannelAddress(this.component.id, 'LockState'),
    ];
  }
  ...
  protected override onCurrentData(currentData: CurrentData) {
    ...
    for (let controller of controllers) {
      ...
      if (
        'evcs.id' in properties &&
        properties['evcs.id'] === this.componentId
      ) {
    ...
    this.isConnectionSuccessful =
      currentData.allComponents[this.component.id + '/State'] != 3
        ? true
        : false;
    ...
    if (
      currentData.allComponents[this.component.id + '/ChargePower'] > 0 ||
    ...
    //console.log(88888, this.isChargingEnabled, 'chargeMode:', this.chargeMode);
    // Mode
    if (this.isChargingEnabled) {
      if (this.chargeMode == 'FORCE_CHARGE') {
        ...
        this.mode = this.translate.instant(
    ...
    if (this.controller) {
      ...
      this.isChargingEnabled = this.controller.properties['enabledCharging']
        ? true
        : false;
      // DefaultChargeMinPower
      this.defaultChargeMinPower =
        this.controller.properties['defaultChargeMinPower'];
      // Prioritization
      this.prioritization =
        this.controller.properties['priority'] in Prioritization
          ? 'Edge.Index.Widgets.EVCS.OptimizedChargeMode.ChargingPriority.' +
            this.controller.properties['priority'].toLowerCase()
          : '';
      // MaxChargingValue
      if (this.phases) {
      ...
      // EnergySessionLimit
      this.energySessionLimit =
        this.controller.properties['energySessionLimit'];
    }
    ...
    this.chargeDischargePower = Utils.convertChargeDischargePower(
      this.translate,
      currentData.allComponents[this.component.id + '/ChargePower']
    );
    this.chargeTarget = Utils.CONVERT_TO_WATT(
      this.formatNumber(
        currentData.allComponents[this.component.id + '/SetChargePowerLimit']
      )
    );
    this.energySession = Utils.CONVERT_TO_WATT(
      currentData.allComponents[this.component.id + '/EnergySession']
    );

    this.minChargePower = this.formatNumber(
      currentData.allComponents[this.component.id + '/MinimumHardwarePower']
    );
    this.maxChargePower = this.formatNumber(
      currentData.allComponents[this.component.id + '/MaximumHardwarePower']
    );
  ...
  private getState(state: number, plug: number): string {
    if (this.controller != null) {
      if (
        this.controller.properties.enabledCharging != null &&
        this.controller.properties.enabledCharging == false
      ) {
        return this.translate.instant(
          'Edge.Index.Widgets.EVCS.chargingStationDeactivated'
        );
      }
    }
    ...
    if (plug == null) {
      ...
      return this.translate.instant(
        'Edge.Index.Widgets.EVCS.cableNotConnected'
      );
    }
    switch (state) {
      ...
      case ChargeState.READY_FOR_CHARGING:
        return this.translate.instant(
          'Edge.Index.Widgets.EVCS.readyForCharging'
        );
      case ChargeState.NOT_READY_FOR_CHARGING:
        return this.translate.instant(
          'Edge.Index.Widgets.EVCS.notReadyForCharging'
        );
      ...
      case ChargeState.ENERGY_LIMIT_REACHED:
        return this.translate.instant(
          'Edge.Index.Widgets.EVCS.chargeLimitReached'
        );
