...
export class StorageComponent extends AbstractFlatWidget {
  ...
  protected possibleBatteryExtensionMessage: Map<
    string,
  ...
  protected override getChannelAddresses() {
    ...
    this.emergencyReserveComponents = this.config
      ...
      //   'io.openems.edge.controller.ess.emergencycapacityreserve.EmergencyCapacityReserve'
      // )
      .filter((component) => component.isEnabled)
      .reduce((result, component) => {
        return {
    ...
    for (let component of this.config
      ...
      if (
        this.config.factories[component.factoryId].natureIds.includes(
  ...
  private getBatteryCapacityExtensionStatus(
    ...
    if (essIsBlocking != null && essIsBlocking == 1) {
      ...
      return {
        color: 'orange',
        text: this.translate.instant('Edge.Index.RETROFITTING.PREPARING'),
      };
    } else {
      return null;
    }
  ...
  protected override onCurrentData(currentData: CurrentData) {
    ...
    for (let essId in this.emergencyReserveComponents) {
      ...
      controller['currentReserveSoc'] =
        currentData.allComponents[controller.id + '/_PropertyReserveSoc'];
      this.isEmergencyReserveEnabled[essId] =
        currentData.allComponents[
          controller.id + '/_PropertyIsReserveSocEnabled'
        ] == 1
          ? true
          : false;
    }
  ...
  public convertPower(value: number, isCharge?: boolean) {
    ...
    if (thisValue > 0) {
      return formatNumber(thisValue, 'en', '1.0-1') + ' kW'; // TODO get locale dynamically
