...
export class ExecuteSystemUpdate {
     ...
     */
    public canNotBeUpdated: boolean = false;//用于标识是否不能在 Windows 上执行更新
    public isEdgeRestarting: boolean = false;//用于标识边缘设备是否正在重新启动。
    public systemUpdateState: SystemUpdateState = { unknown: {} };//存储系统更新状态的对象
    private ngUnsubscribe = new Subject<void>();//一个 RxJS Subject，用于取消订阅
    ...
    private stopRefreshSystemUpdateState() {
        this.ngUnsubscribe.next();//next 信号来中断 RxJS 流停止更新系统更新状态
    }
