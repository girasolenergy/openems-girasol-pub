...
export class ChannelsComponent {
  ...
  protected componentChannelConfig: Map<String, EdgeConfig.ComponentChannel> =
    new Map();
  constructor(
  ...
  protected subscribeChannel(componentId: string, channelId: string): void {
    ...
    if (
      this.componentChannels[componentId]?.filter(
        (element) => element.channelId == address.channelId
      )?.length === 0
    ) {
    ...
    this.componentChannelConfig.set(
      address.toString(),
      this.config.getChannel(address)
    );
  ...
  protected unsubscribeChannel(channelAddress: ChannelAddress): void {
    this.componentChannels[channelAddress.componentId] = this.componentChannels[
    ...
    this.channelsToBeSubscribed.forEach((item, index) => {
      if (
        item.componentId === channelAddress.componentId &&
        item.channelId === channelAddress.channelId
      ) {
  ...
  private saveChannels(): void {
    let data = Object.entries(this.channelsToBeSubscribed)
      .map(([componentId, channels]) => {
        return channels.toString();
      })
      .toString();
    this.router.navigate(['device/' + this.edge.id + '/settings/channels/'], {
      queryParams: { save: data },
    });
  }
  ...
  protected localSave() {
    ...
    localStorage.setItem(
      ChannelsComponent.URL_PREFIX + '-' + this.edge.id,
      dataStr
    );
  ...
  protected loadSavedChannels() {
    ...
    if (address) {
      this.channels = address
        .split(',')
        ?.map((element) => ChannelAddress.fromString(element));
      this.channels.map((el) =>
        this.subscribeChannel(el.componentId, el.channelId)
      );
    } else if (storedValue) {
