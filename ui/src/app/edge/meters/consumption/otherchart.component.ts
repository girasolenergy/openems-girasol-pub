...
export class ConsumptionOtherChartComponent
  ...
  implements OnInit, OnChanges, OnDestroy
{
      ...
      .then((response) => {
        this.service
          ...
          .then((config) => {
            ...
            otherConsumption = result.data['_sum/ConsumptionActivePower'].map(
              (value, index) => {
                if (value != null) {
            ...
            if (
              ...
              totalMeteredConsumption.length > 0
            ) {
              datasets.push({
  ...
  protected getChannelAddresses(
    ...
    return new Promise((resolve) => {
      ...
      config
        .getComponentsImplementingNature('io.openems.edge.evcs.api.Evcs')
        .filter((component) => !(component.factoryId == 'Evcs.Cluster'))
        .forEach((component) => {
