  ...
  implements OnInit, OnChanges, OnDestroy
{
  ...
  public checkIfToday(): boolean {
    const dateYesterday = this.service.historyPeriod.value.to.getDate();
    ...
    this.service.getCurrentEdge().then((edge) => {
      this.service.getConfig().then((config) => {
        this.getEnergyChannelAddresses(config).then((channelAddresses) => {
          const request = new QueryHistoricExportXlxsRequest(
          ...
          edge
            ...
            .then((response) => {
              ...
              if (isSameDay(dateFrom, dateTo)) {
                fileName += format(dateFrom, "yyyy-MM-dd");
            } else if (isSameMonth(dateFrom, dateTo)) {
                fileName += format(dateFrom, "yyyy-MM-dd") + "-" + format(dateTo, "yyyy-MM-dd");
  ...
  public getConsumptionData(index: number): number {
    const data = (this.consumptionDatasets[index]?.data as number[])?.filter((d) => d !== 0);
    return data?.length > 0 ? data[data.length - 1] : 0;
  }
  public getProductionData(index: number): number {
    const data = (this.productionDatasets[index]?.data as number[])?.filter((d) => d !== 0);
    return data?.length > 0 ? data[data.length - 1] : 0;
  }
      ...
      this.queryHistoricTimeseriesDataMeter(
        ...
        this.meterIds?.forEach((meterId) => {
          if (`${meterId}/ActiveConsumptionEnergy` in result.data) {
            const consumptionData = result?.data[`${meterId}/ActiveConsumptionEnergy`].map((value) => {
              return value != null ? value / 1000 : 0; // convert to kWh
            }) || [];
            this.consumptionDatasets.push({ data: consumptionData ?? [] });
          }
          if (result && result.data && `${meterId}/ActiveProductionEnergy` in result.data)  {
            const productionData = result?.data[`${meterId}/ActiveProductionEnergy`]?.map((value) => {
