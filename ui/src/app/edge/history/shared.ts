...
export function calculateActiveTimeOverPeriod(
  channel: ChannelAddress,
  queryResult: QueryHistoricTimeseriesDataResponse['result']
) {
  let startDate = startOfDay(new Date(queryResult.timestamps[0]));
  let endDate = new Date(
    queryResult.timestamps[queryResult.timestamps.length - 1]
  );
  let activeSum = 0;
...
export function calculateResolution(
  ...
  fromDate: Date,
  toDate: Date
): { resolution: Resolution; timeFormat: 'day' | 'month' | 'hour' } {
  let days = Math.abs(differenceInDays(toDate, fromDate));
  let resolution: {
    resolution: Resolution;
    timeFormat: 'day' | 'month' | 'hour';
  };

  if (days <= 1) {
    resolution = {
      resolution: { value: 1, unit: Unit.MINUTES },
      timeFormat: 'hour',
    }; // 5 Minutes/1
  } else if (days == 2) {
    if (service.isSmartphoneResolution) {
      resolution = {
        resolution: { value: 1, unit: Unit.DAYS },
        timeFormat: 'hour',
      }; // 1 Day
    } else {
      resolution = {
        resolution: { value: 10, unit: Unit.MINUTES },
        timeFormat: 'hour',
      }; // 1 Hour
    }
  } else if (days <= 4) {
    if (service.isSmartphoneResolution) {
      ...
      resolution = {
        resolution: { value: 1, unit: Unit.HOURS },
        timeFormat: 'hour',
      }; // 1 Hour
    }
  } else if (days <= 6) {
    ...
    if (service.isSmartphoneResolution == true) {
      resolution = {
        resolution: { value: 1, unit: Unit.MONTHS },
        timeFormat: 'month',
      }; // 1 Month
    } else {
      resolution = {
