...
export class DelayedSellToGridChartComponent
    ...
    this.queryHistoricTimeseriesData(this.period.from, this.period.to)
      .then((response) => {
        this.service
          .getConfig()
          .then((config) => {
            let meterIdActivePower =
              config.getComponent(this.componentId).properties['meter.id'] +
              '/ActivePower';
            let sellToGridPowerLimit =
              this.componentId + '/_PropertySellToGridPowerLimit';
            let continuousSellToGridPower =
              this.componentId + '/_PropertyContinuousSellToGridPower';
            ...
            if ('_sum/EssActivePower' in result.data) {
              ...
              if (
                ...
                effectivePower = result.data[
                  '_sum/ProductionDcActualPower'
                ].map((value, index) => {
                  return Utils.subtractSafely(
                    result.data['_sum/EssActivePower'][index],
                    value
                  );
                });
              } else {
