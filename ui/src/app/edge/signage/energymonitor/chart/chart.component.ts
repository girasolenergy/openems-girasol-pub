...
export class EnergymonitorChartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.service.currentEdge.subscribe((edge) => {
      if (edge) {
        ...
        let request = new getEdgeUimiscRequest({ edgeId: edgeId });
  ...
  updateCurrentData(currentData: CurrentData) {
    ...
    let { system, storage, production, grid, consumption } = currentData.summary;

    if (this.proLimit) {
      production.powerRatio = production.activePower / this.proLimit;
    }

    let updatedSummary = { system, storage, production, grid, consumption };
