...
import { getEdgeUimiscRequest } from "src/app/shared/jsonrpc/request/getEdgeUimiscRequest";
import { getEdgeUimiscResponse } from "src/app/shared/jsonrpc/response/getEdgeUimiscResponse";
...
export class EnergymonitorChartComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.service.currentEdge.subscribe((edge) => {
      if (edge) {
        ...
        const request = new getEdgeUimiscRequest({ edgeId: edgeId });
        this.websocket.sendSafeRequest(request).then((response) => {
          this.proLimit = (response as getEdgeUimiscResponse).result.edge[2]?.value;
        });
      }
    });
  }
  ...
  updateCurrentData(currentData: CurrentData) {
    ...
    const { system, storage, production, grid, consumption } = currentData.summary;

    if (this.proLimit) {
      production.powerRatio = production.activePower / this.proLimit;
    }

    const updatedSummary = { system, storage, production, grid, consumption };

    [this.consumptionSection, this.gridSection, this.productionSection, this.storageSection]
      .filter(section => section != null)
      .forEach(section => {
        section.updateCurrentData(updatedSummary);
      });
  }
  ...
  private updateOnWindowResize(): void {
    ...
    if (this.chartDiv.nativeElement.offsetParent) {
      size = this.chartDiv.nativeElement.offsetParent.offsetWidth - 200;
    }
