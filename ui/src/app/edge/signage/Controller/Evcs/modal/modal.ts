...
export class ModalComponent extends AbstractModal{
  ...
  updateChargeMode(
    ...
    const oldEnabledCharging = currentController.properties.enabledCharging;
  ...
  */
  updateEnergySessionLimit(
    ...
    const oldLimit = currentController.properties.energySessionLimit;
    const newLimit = event.detail.value * 1000;
 ...
 */
  ...
  protected override onCurrentData(currentData: CurrentData) {
    ...
    if (this.formGroup?.pristine) {
      this.lockState=currentData.allComponents[this.component.id + "/LockState"];
  ...
  protected override getFormGroup(): FormGroup {
     return this.formBuilder.group({
      chargeMode: new FormControl(this.controller?.properties.enabledCharging == false ? "OFF" : this.controller?.properties.chargeMode),
       ...
       enabledCharging: [this.isChargingEnabled],
     });
   }
  ...
  protected override onIsInitialized(): void {
    ...
    this.subscription.add(this.formGroup?.controls["chargeMode"].valueChanges.subscribe(chargeMode => {
      if (chargeMode === "OFF") {
        ...
        this.formGroup.controls["chargeMode"].value.setValue(chargeMode);
      }
      this.formGroup.get("enabledCharging").markAsDirty();
    }));

    this.subscription.add(this.formGroup?.controls["minGuarantee"].valueChanges.subscribe(minGuarantee => {
      if (minGuarantee) {
